...
const std::string METEOR_PATH = "sounds/explosion.ogg";
...
void callJavaMethod4(std::string func, std::string arg0)
{
	JniMethodInfo t;
...
bool Stage3::init()
{
	...
	howTo = Sprite::create("Images/stage3HowTo.png");
...
void Stage3::soundOnoff(int i)
{
	...
	if (i == 0)
	{
		if (b)
		{
			CSoundManager::getInstance()->playBgm(OVER_PATH);
		}
		else
		{
	...
	else if (i == 1)
	{
		if (b)
		{
			CSoundManager::getInstance()->playBgm(CLEAR_PATH);
		}
		else
		{
	...
	else if (i == 7)
	{
		if (e)
		{
			CSoundManager::getInstance()->playEffect(METEOR_PATH);
		}
		else
		{
			CSoundManager::getInstance()->stopEffect(METEOR_PATH);
		}
	}
}
...
void Stage3::overBestScore(float f)
{
	...
	if (nowScore == nScore)
	{
		...
		#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
				callJavaMethod4("SendStage3", str);
		#endif
		}
...
void Stage3::BeginContact(b2Contact *contact)
{
	...
	if (bodyA->GetType() == b2_dynamicBody && bodyB->GetType() == b2_staticBody && nTag == 6)
	{
		...
		else if (dBool && swichNum == 4)
		{
			...
			UserDefault::getInstance()->setIntegerForKey("stage", 4);
